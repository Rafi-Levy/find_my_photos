      ...
      if (elements.enrolledProfilesContainer && children.length > 0) {
        ...
        elements.enrolledProfilesList.innerHTML = children.map(c => `
          <span class="profile-pill">
            <svg class="profile-avatar-icon" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M19 21v-2a4 4 0 0 0-4-4H9a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>
            <strong>${escapeHtml(c.name)}</strong> <span class="profile-count">(${c.photosUsed} photos)</span>
