...
class ForwarderEngine extends EventEmitter {
  ...
  reloadReference() {
    try {
      if (fs.existsSync(this.config.referencePath)) {
        this.referenceDescriptor = loadReference(this.config.referencePath);
        this.log('info', 'Child face profile loaded successfully.');
        this.emit('reference_updated', this.getReferenceInfo());
        return true;
      }
    } catch (err) {
      this.log('warn', `Could not load reference embedding: ${err.message}`);
    }
    this.referenceDescriptor = null;
    return false;
  }
  ...
  getReferenceInfo() {
    ...
    try {
      const data = JSON.parse(fs.readFileSync(this.config.referencePath, 'utf-8'));
      return {
        enrolled: true,
        createdAt: data.createdAt,
        photosUsed: data.photosUsed,
  ...
  async processImage(msg, msgId) {
    ...
    const result = anyFaceMatches(descriptors, this.referenceDescriptor, this.config.matchThreshold);
    ...
    if (result.matched) {
      ...
      this.log('info', `Child face matched in photo! (${( (1 - result.distance) * 100).toFixed(0)}% match) — ${forwarded ? 'Forwarded' : 'Saved to Review'}.`);
      this.emit('stats', this.statsTracker.getStats());
  ...
  async processVideo(msg, msgId) {
    ...
    try {
      const matched = await withExtractedFrames(
        ...
        async (framePaths) => {
          ...
          for (let i = 0; i < framePaths.length; i++) {
            ...
            if (descriptors.length > 0) {
              const res = anyFaceMatches(descriptors, this.referenceDescriptor, this.config.matchThreshold);
              if (res.bestDistance !== null && res.bestDistance < globalMinDistance) {
                globalMinDistance = res.bestDistance;
              }
              if (res.matched) {
                return { matched: true, distance: res.distance, frame: i + 1 };
              }
            }
          }
      ...
      if (matched.matched) {
        ...
        this.log('info', `Child face matched in video! (${( (1 - matched.distance) * 100).toFixed(0)}% match) — ${forwarded ? 'Forwarded' : 'Saved to Review'}.`);
        this.emit('stats', this.statsTracker.getStats());
        this.emit('match', matchEntry);
      } else {
  ...
  async runTraining(onProgress) {
    ...
    this.referenceDescriptor = averaged;

    this.log('info', `Training complete! Face profile saved based on ${descriptors.length} clear photos.`);
    this.emit('status', this.getStatus());
