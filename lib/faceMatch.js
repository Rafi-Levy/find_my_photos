...
/**
 * Ensure required face-api models are present on disk, copying from npm package if missing.
 */
function ensureModelsExist(modelsDir) {
  const files = [
  ...
  const hasMissing = files.some(f => !fs.existsSync(path.join(modelsDir, f)));
  if (hasMissing) {
    try {
      const pkgModelDir = path.join(
        path.dirname(require.resolve('@vladmandic/face-api/package.json')),
        'model'
      );
      for (const file of files) {
        const dest = path.join(modelsDir, file);
        const src = path.join(pkgModelDir, file);
        if (!fs.existsSync(dest) && fs.existsSync(src)) {
          fs.copyFileSync(src, dest);
        }
      }
    } catch (err) {
      // Fall through to faceapi loading, which will report any missing files
    }
  }
}
